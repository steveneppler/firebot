...
from .sources.firms import Hotspot, satellite_name
...
def build_firms_embed(
    ...
    *,
    ...
    map_url = firms_map_url(hs.lat, hs.lon, zoom)
    conf = _VIIRS_CONF_LABELS.get(str(hs.confidence).strip().lower(), str(hs.confidence) or "n/a")
    fields = [
        {"name": "Confidence", "value": conf, "inline": True},
        {"name": "Satellite", "value": satellite_name(hs.satellite), "inline": True},
    ]
    if hs.frp is not None:
        fields.append({"name": "FRP", "value": f"{hs.frp:.1f} MW", "inline": True})
    if center_lat is not None and center_lon is not None and center_label:
        fields.append({"name": "Distance",
                       "value": offset_description(center_lat, center_lon, hs.lat, hs.lon, center_label),
                       "inline": True})
    fields.append({"name": "Detected", "value": _fmt_dt(hs.acq_dt), "inline": True})

    embed = {
        "title": "🛰️ Satellite fire hotspot",
        "url": map_url,
        "color": COLOR_HOTSPOT,
        "description": f"📍 [{hs.lat:.4f}, {hs.lon:.4f}]({map_url}) · unconfirmed detection · 🗺️ NASA FIRMS map",
        "fields": fields,
        "footer": {"text": "Source: NASA FIRMS"},
    }
    if static_map is not None:
        img = static_map(hs.lat, hs.lon)
        if img:
            embed["image"] = {"url": img}
    return embed
...
def post_embeds(webhook_url: str, embeds: list[dict], *, timeout: int = 30) -> None:
    ...
    for i in range(0, len(embeds), MAX_EMBEDS_PER_MESSAGE):
        ...
        resp = requests.post(webhook_url, json={"embeds": batch}, timeout=timeout)
        # 429 = rate limited; honor retry_after then retry once.
        if resp.status_code == 429:
            retry_after = resp.json().get("retry_after", 1) if resp.content else 1
